...
with delay as (select
                 ...
                 where (unit_type = 'ZONE_ANSP' and unit_id in (55, 57)) or
                    (unit_type = 'ZONE' and unit_id=13)
...
traffic as ( select
              ...
              where (type = 'ZONE_ANSP' and id in (55, 57)) or
                    (type = 'ZONE' and id=13)
...
select
   ...
   , case d.unit_name
        when 'CFMU AREA' then 'EUROCONTROL Area (MS)'
        when 'SES Performance Scheme SES-RP1 based on ANSP' then 'SES Area (RP1)'
        when 'SES Performance Scheme SES-RP2 based on ANSP(AUA)' then 'SES Area (RP2)'
    end ENTITY_NAME
